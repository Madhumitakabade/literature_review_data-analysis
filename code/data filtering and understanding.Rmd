...
# (Sum of the 6 group columns: Copepods + Euphausiids + Amphipods + etc.)
df <- df %>%
...
ggplot(df, aes(x = Region, y = Total_Mesozooplankton, fill = Region)) +
  ...
  labs(
    ...
    x = "Region",
    y = "Total Mesozooplankton Abundance (log10 scale)"
  ) +
  scale_y_log10() +    # << ADD THIS
  scale_fill_manual(values = c("Great Australian Bight" = "skyblue", "Bass Strait" = "lightgreen")) +
  theme_minimal() +
  theme(
...
# Step 1: Pivot longer to get one row per species per station
df_long <- df %>%
  pivot_longer(cols = c(`Copepods`, `Euphausiids`, `Amphipods_Mysids`,
                        `Fish_Larvae_Eggs`, `Gelatinous_Zooplankton`, `Other_Crustaceans`),
               names_to = "Group", values_to = "Abundance")

# Step 2: Add biomass per group using estimated weights (adjustable)
weights <- c(
  "Copepods" = 0.02,
  "Euphausiids" = 3,
  "Amphipods_Mysids" = 5,
...
# Step 3: Summarise abundance and biomass by region and group
...
# Step 4a: Plot ABUNDANCE
ggplot(region_summary, aes(x = Group, y = Total_Abundance, fill = Region)) +
  geom_col(position = "dodge") +
  labs(title = "Mesozooplankton Abundance by Group and Region",
       x = "Functional Group", y = "Total Abundance (ind/m³)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 30, hjust = 1))

# Step 4b: Plot BIOMASS
ggplot(region_summary, aes(x = Group, y = Total_Biomass, fill = Region)) +
  geom_col(position = "dodge") +
  labs(title = "Mesozooplankton Biomass by Group and Region",
       x = "Functional Group", y = "Estimated Biomass (mg/m³)") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 30, hjust = 1))
...
# Make sure PCI is numeric
df_long$PCI <- as.numeric(as.character(df_long$PCI))

# Group and smooth (optional)
smoothed_df <- df_long %>%
  group_by(PCI, Group) %>%
  summarise(
...
# Plot
ggplot(df_long, aes(x = PCI, y = Abundance, color = Group)) +
  geom_point(alpha = 0.3, size = 1) +  
  geom_smooth(method = "loess", se = TRUE, linewidth = 1.2) +  
  scale_color_viridis_d(option = "D", end = 0.8) +
  scale_y_log10() +  # <<<<<< THIS IS THE NEW LINE
  labs(
    x = "Productivity Classification Index (PCI)",
    y = "Abundance (log10 scale, ind/m³)",
    title = "Smoothed Abundance Trends Across PCI Categories",
...
tukey_combined <- map_df(unique(df_long$FunctionalGroup), function(grp) {
  df_sub <- df_long %>% filter(FunctionalGroup == grp)
...
# View and/or export
View(summary_tukey_final)
# write.csv(summary_tukey_final, "summary_stat_table.csv", row.names = FALSE)
...
# Make the pie chart
ggplot(focus_data, aes(x = "", y = Papers, fill = Focus)) +
...
ggplot(pie_data, aes(x = "", y = Papers, fill = Region)) +
  geom_col(width = 1, color = "white") +
  coord_polar(theta = "y") +
  labs(title = "Distribution of Literature by Regional Focus") +
  theme_void() +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 16),
    legend.title = element_blank()
  ) +
  geom_text(aes(label = paste0(Papers)), 
